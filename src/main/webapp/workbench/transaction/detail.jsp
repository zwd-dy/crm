<%@ page import="me.zwdi.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="me.zwdi.crm.workbench.domain.Tran" %>
...
<script type="text/javascript">
	...
	function changeStage(stage,i){
		$.ajax({
			url: "workbench/transaction/updateChangeStage.do",
			data: {
				"id" : "${t.id}",
				"stage": stage,
			...
			success: function(data){
				...
				if(data.success){
					...
					// 改变阶段成功后，将所有的阶段图标重新判断，重新赋予样式以及颜色
					changeIcon(stage,i);
	...
	function changeIcon(stage,index1){
		...
		// 如果当前阶段的可能性为0，前七个一定是黑圈，后两个一个是红X，一个是黑X
		if(currentPossibility == "0"){
			...
			// 遍历后两个
			for (var i = point-1; i < <%=dvList.size()%>; i++) {
			...
			// 如果当前阶段的可能性不为0 前七个绿圈，绿色标记，黑圈，后两个一定是黑X
		} else{
			// 遍历前七个
			for (var i = 0; i < point; i++) {
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		%>
					<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')"
						  class="glyphicon glyphicon-remove mystage mystage"
						  data-toggle="popover"
		...
		<%
				...
				// 如果当前阶段可能性不为0，前七个有可能是绿圈，绿色标记，黑圈，后两个一定是黑X
				// 准备当前阶段的下标
				int index = 0;
				for (int i = 0; i < dvList.size(); i++) {
